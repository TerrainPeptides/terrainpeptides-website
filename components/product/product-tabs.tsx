...
function ProductFaqs({ faqs }: { faqs: FAQ[] }) {
        ...
        <CardContent className="pt-0">
          <div className="overflow-hidden rounded-xl border border-border">
            <Accordion type="single" collapsible className="w-full">
              {faqs.map((faq) => (
                <AccordionItem key={faq.id} value={faq.id} className="px-4 sm:px-5">
                  <AccordionTrigger className="py-4 text-left text-sm font-medium text-foreground hover:no-underline [&>svg]:text-muted-foreground [&[data-state=open]>svg]:text-foreground/70 sm:py-5">
                    {faq.question}
                  </AccordionTrigger>
...
export function ProductTabs({ product, researchContent, hasFullResearch = false, faqs = [] }: ProductTabsProps) {
  return (
    <div className="mt-16">
