...
import { ChevronLeft, FileText, Star, FlaskConical, Beaker, Package, BookOpen, HelpCircle, ExternalLink } from 'lucide-react'
import { RelatedProducts } from '@/components/product/related-products'
import { ProductNamePanel } from '@/components/product/product-name-panel'
import type { Product, Vouch } from '@/lib/types'

interface RetatrutideProductProps {
  product: Product
...
export function RetatrutideProduct({ product, relatedProducts = [], vouches = [] }: RetatrutideProductProps) {
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="research" className="mt-0 space-y-12 text-[1.1em]">
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm text-[#0A1931]/60">Triple incretin-glucagon co-agonism.</p>
                <div className="mt-4 grid gap-6 lg:grid-cols-5">
                  <Card className="border-[#0A1931]/10 lg:col-span-3">
                    <CardContent className="p-6">
                      ...
                      <h3 className="mt-2 font-bold text-[#0A1931]">Triple Receptor Co-Agonism</h3>
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm text-[#0A1931]/60">Summary of preclinical and clinical findings.</p>
                <div className="mt-4 grid gap-6 lg:grid-cols-2">
                  <Card className="border-[#0A1931]/10">
                    <CardContent className="p-6">
                      <span className="rounded-full bg-[#0A1931]/5 px-3 py-1 text-xs text-[#0A1931]/70">🗓️ Phase 2 Obesity Trial — Adults with Overweight/Obesity</span>
                      <p className="mt-4 text-4xl font-bold" style={{ color: GREEN }}>≈24-26% reduction</p>
                      <p className="text-xs font-medium uppercase tracking-wider text-[#0A1931]/70">Mean body weight loss at top dose after ~1 year</p>
                      <p className="mt-4 text-sm font-semibold text-[#0A1931]">Weight loss comparison</p>
                        ...
                        {COMPARISON_ROWS.map((r) => (
                          <div key={r.label} className="flex items-center gap-3">
                            ...
                            <div className="h-3 flex-1 overflow-hidden rounded-full bg-[#0A1931]/10">
                              <div className="h-full rounded-full" style={{ width: `${r.bar}%`, backgroundColor: r.bar === 100 ? GREEN : r.bar > 50 ? 'rgba(22,163,74,0.8)' : '#0A1931' }} />
                            </div>
                            <span className="text-xs font-medium" style={{ color: r.bar === 100 ? GREEN : undefined }}>{r.value}</span>
                          </div>
                        ))}
                      </div>
                      <p className="mt-4 text-xs font-medium text-[#0A1931]/70">Trial details:</p>
                      <div className="mt-1 rounded-lg bg-[#0A1931]/5 p-3 text-xs text-[#0A1931]/60">
                        ...
                        {KEY_TRIAL_RESULTS.map((t) => (
                          <div key={t.label}>
                            <div className="flex justify-between text-sm">
                              <span className="font-bold text-[#0A1931]" style={{ color: GREEN }}>{t.value}</span>
              ...
              <div>
                <p className="text-xs font-medium uppercase tracking-wider text-[#0A1931]/60">✨ Beyond weight loss</p>
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm text-[#0A1931]/60">Reported effects from clinical trials.</p>
                <div className="mt-4 grid gap-6 lg:grid-cols-2">
                  <Card className="border-[#0A1931]/10">
                    <CardContent className="p-6">
                ...
                <div className="mt-4 grid gap-6 lg:grid-cols-3">
                  <Card className="border-[#0A1931]/10">
                    <CardContent className="p-6">
                      ...
                      <h4 className="mt-3 font-bold text-[#0A1931]">What Is Retatrutide?</h4>
                      <div className="mt-4 rounded-lg bg-[#0A1931]/5 p-4 space-y-2 text-sm">
                        <div className="flex justify-between"><span className="text-[#0A1931]/60">Type</span><span className="font-medium">Synthetic peptide triple agonist (GIP/GLP-1/glucagon receptor agonist)</span></div>
                        <div className="flex justify-between"><span className="text-[#0A1931]/60">CAS</span><span className="font-medium">Not yet publicly standardized — confirm before publishing</span></div>
                        <div className="flex justify-between"><span className="text-[#0A1931]/60">Formula</span><span className="font-medium">Proprietary large peptide analog</span></div>
                        <div className="flex justify-between"><span className="text-[#0A1931]/60">Weight</span><span className="font-medium">High-kilodalton peptide — confirm from primary source</span></div>
                        <div className="flex justify-between"><span className="text-[#0A1931]/60">Amino acids</span><span className="font-medium">Modified sequence engineered for triple receptor affinity and extended half-life</span></div>
                        <div className="flex justify-between"><span className="text-[#0A1931]/60">Sequence</span><span className="font-medium">Proprietary — not generally reported in clinical trial summaries</span></div>
                      </div>
                    </CardContent>
                  </Card>
                  <Card className="border-[#0A1931]/10">
                    <CardContent className="p-6">
                      ...
                      <div className="mt-4 space-y-3">
                        <div className="rounded-lg bg-[#0A1931]/5 p-3 flex items-center gap-2">
                          <span className="text-sm text-[#0A1931]/80">Storage:</span>
                          <span className="text-sm font-medium">Refrigerated sterile solution</span>
                        </div>
                        <div className="rounded-lg bg-[#0A1931]/5 p-3 flex items-center gap-2">
                          <span className="text-sm text-[#0A1931]/80">Protect:</span>
              ...
              <div>
                <div className="flex items-center gap-2">
                  <BookOpen className="h-5 w-5 text-[#0A1931]/60" />
                  <p className="text-xs font-medium uppercase tracking-wider text-[#0A1931]/60">Sources & References</p>
                </div>
                <p className="mt-0.5 text-sm text-[#0A1931]/60">Peer-reviewed research.</p>
                <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  {SOURCES.map((s) => (
                    <Card key={s.title} className="border-[#0A1931]/10 transition-transform duration-150 ease-out hover:scale-[1.02]">
                      <CardContent className="p-5">
                        <p className="text-xs font-medium uppercase tracking-wider" style={{ color: GREEN }}>{s.journal}</p>
                        <h4 className="mt-2 font-bold text-[#0A1931]">{s.title}</h4>
                        <p className="mt-1 text-xs text-[#0A1931]/60">{s.meta}</p>
                        <p className="mt-1 text-xs italic text-[#0A1931]/60">{s.authors}</p>
                        {s.url && (
              ...
              <div>
                <div className="flex items-center gap-2">
                  <HelpCircle className="h-5 w-5 text-[#0A1931]/60" />
                  <p className="text-xs font-medium uppercase tracking-wider text-[#0A1931]/60">Frequently Asked Questions</p>
                </div>
                <p className="mt-0.5 text-sm text-[#0A1931]/60">Common questions about Retatrutide research.</p>
                <div className="mt-4 space-y-4">
                  {FAQ_ITEMS.map((faq) => (
                    <Card key={faq.q} className="border-[#0A1931]/10">
                      <CardContent className="p-6">
                        <h4 className="font-bold text-[#0A1931]">{faq.q}</h4>
                        <p className="mt-2 text-sm text-[#0A1931]/80">{faq.a}</p>
                      </CardContent>
                    </Card>
                  ))}
                </div>
